/* 舒适圈今天页日夜两态截图：验证「天黑了 / 进入明天」换段键的上下间距。 */
...
const chrome = spawn(CHROME, [
  ...
  '--user-data-dir=' + path.join(process.cwd(), '.cdp-profile-shiftbtn'),
], { stdio: 'ignore' });
...
const mkSave = (dayPhase) => JSON.stringify({
  sessionId: 'run_shiftbtn', rngSeed: 777, day: 12, daysLimit: 30,
  phase: 'main', dayPhase,
  ...
  chat: null, log: [
    { day: 11, kind: 'comfort', line: '', details: '常用手机 · 妈的生活费收下了，家里 74。' },
    { day: 11, kind: 'comfort', line: '', details: '常用手机 · 曼曼发来下午茶战报。' },
  ], usedOneTimeEvents: [],
  ...
  comfort: { active: true, family: 74, love: 88, minutes: 30, blockedByBf: false, bfState: 'normal', datesMet: { wu: 8, chen: 10 }, momGiven: 460, bfGiven: 160, bfTaken: 260, pending: '', dayAck: 12 },
});

for (const [phase, name] of [['morning', 'comfort-shift-day'], ['night', 'comfort-shift-night']]) {
  await send('Page.navigate', { url: BASE });
  await waitFor(`!!document.querySelector('.title-buttons .btn')`, 'title-' + phase);
  await evaljs(`localStorage.setItem('beng_save_v1', ${JSON.stringify(mkSave(phase))})`);
  await send('Page.navigate', { url: BASE });
  await waitFor(`!!document.querySelector('.title-buttons .btn')`, 'title2-' + phase);
  ...
  await waitFor(`!!document.querySelector('.comfort-today')`, 'today-' + phase, 12000);
  await sleep(700);
  await evaljs(`[...document.querySelectorAll('.comfort-today > .btn')].at(-1)?.scrollIntoView({ block: 'center' })`);
  await sleep(400);
