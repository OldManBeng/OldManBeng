/* 舒适圈 UI 精简任务的实景验收截图：注入存档 → 今天页 / 通讯录（收起+展开）。 */
...
const chrome = spawn(CHROME, [
  ...
  '--user-data-dir=' + path.join(process.cwd(), '.cdp-profile-uishots'),
], { stdio: 'ignore' });
...
const save = JSON.stringify({
  sessionId: 'run_uishots', rngSeed: 777, day: 12, daysLimit: 30,
...
await waitFor(`!!document.querySelector('.comfort-theme')`, 'comfort', 12000);
await sleep(500);
await click('开始今天');
await sleep(500);
await screenshot('ui-after-today');
// 通讯录：收起态
await evaljs(`[...document.querySelectorAll('.nav-btn')].find(b => b.textContent.includes('通讯录'))?.click()`);
await sleep(500);
await screenshot('ui-after-contacts');
// 展开妈一行
await evaljs(`[...document.querySelectorAll('.comfort-contact-row')].find(r => r.textContent.includes('王秀兰'))?.click()`);
await sleep(400);
await screenshot('ui-after-contacts-open');
console.log('DONE');
